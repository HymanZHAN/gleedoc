...
pub fn add_1_test() {
  let result = add(1, 2)
  assert result == 3 as "dev/fixtures/example.gleam:8"
}

// From: dev/fixtures/example.gleam:16
pub fn multiply_2_test() {
  let result = multiply(3, 4)
  assert result == 12 as "dev/fixtures/example.gleam:18"
}

// From: dev/fixtures/example.gleam:26
pub fn greet_3_test() {
  let msg = greet("Alice")
  assert msg == "Hello, Alice!" as "dev/fixtures/example.gleam:28"
}
...
pub fn find_4_test() {
  ...
  assert users |> find("hello") == User("", "")
    as "dev/fixtures/example.gleam:48"
  assert users |> find("john_doe") == john as "dev/fixtures/example.gleam:49"
}
